...
export default function VacationsChart(): JSX.Element {
    ...
    function getVacationStats(
        ...
        for (const vacation of vacations) {
            const { description, name } = vacation;
            let countryStats = stats.find((s) => s.description === description);

            if (!countryStats) {
                countryStats = { description, name: [name], followers: 0, users: [] };
                stats.push(countryStats);
            } else if (!countryStats.name.includes(name)) {
                countryStats.name.push(name);
            }

            const vacationFollowers = followers.filter(
                (f) => f.vacationId === vacation.vacationId
            );
            followerCount[description] =
                (followerCount[description] || 0) + vacationFollowers.length;
        ...
        for (const stat of stats) {
            const followersForCountry = followersWithUsers.filter((follower) => {
                ...
                return (
                    vacation &&
                    vacation.description === stat.description &&
                    stat.name.includes(vacation.name)
                );
            });
            //   stat.users = followersForCountry;
            stat.followers = followerCount[stat.description] || 0;
        }
