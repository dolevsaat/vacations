...
function DestinationCSV(props: DestinationCSVProps): JSX.Element {
    ...
    function createCSV(): void {
        ...
        const mergedData = newData.reduce(
            (
                acc: { name: string; followers: { id: number; count: number }[] }[],
                cur
            ) => {
                const existingName = acc.find((el) => el.name === cur.name);
                if (existingName) {
                    const existingFollower = existingName.followers.find(
                        (follower: { id: number; count: number }) => follower.id === cur.id
                    );
                    if (existingFollower) {
                        existingFollower.count += cur.followers;
                    } else {
                        existingName.followers.push({ id: cur.id, count: cur.followers });
                    }
                } else {
                    acc.push({
                        name: cur.name,
